...
"""
when the frontend sends the oauth code this api exchange it for the token and attaches
a jwt token for each subsequent requests this helps preventing the token tampering
or prevents the user from manipulating tokens at client side via localstorage
"""
...
def login():
    ...
    try:
        ...
        # Get user info like name,email ,picture with the access token
...
def logout():
    ...
    response.delete_cookie('access_token_cookie', path='/') # delete the cookie so user is logged out and cant access protected routes
    return response